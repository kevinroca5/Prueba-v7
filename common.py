...
@st.cache_data
def list_team_files():
    return {p.stem: str(p) for p in sorted(MATCHES_DIR.glob("*.csv"))}
...
def render_match_selection_sidebar(team_name, matches, key_prefix):
    ...
    venue_filter = st.sidebar.multiselect("Local / Visitante", ["Local", "Visitante"],
                                           default=["Local", "Visitante"], key=f"{key_prefix}_venue")
    ...
    if has_coach_column:
        ...
        with st.sidebar.expander("➕ Añadir tramo de entrenador"):
            c_name = st.text_input("Nombre del entrenador", key=f"{key_prefix}_coach_name_input")
            c_start = st.date_input("Desde", key=f"{key_prefix}_coach_start_input",
                                     value=pd.to_datetime(min_date).date())
            c_end = st.date_input("Hasta", key=f"{key_prefix}_coach_end_input",
                                   value=pd.to_datetime(max_date).date())
            if st.button("Añadir tramo", key=f"{key_prefix}_coach_add_btn"):
                if c_name:
        ...
        if st.session_state.coach_ranges:
            ...
            coach_filter = st.sidebar.multiselect("Filtrar por entrenador", coach_names, default=coach_names,
                                                   key=f"{key_prefix}_coach_manual")
            if st.sidebar.button("Borrar tramos de entrenador", key=f"{key_prefix}_coach_clear_btn"):
                st.session_state.coach_ranges = []
                st.rerun()
    ...
    selected_labels = st.sidebar.multiselect("Selecciona los partidos a incluir", match_labels,
                                              default=match_labels, key=f"{key_prefix}_match_select")
